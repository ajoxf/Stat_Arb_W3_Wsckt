...
def test_use_market_decision_matrix():
    ...
    def use_market(reason, maker_attempts, MAX_ATTEMPTS=1, postonly_rejects=0, MARKET_AFTER=1):
        r = reason.upper()
    ...
    # Reversion / target / max-hold: maker-first via the POST_ONLY path.
    assert use_market("EXIT", maker_attempts=0, postonly_rejects=0) is False
    assert use_market("PROFIT_TARGET", maker_attempts=0, postonly_rejects=1) is True
